function preload () {
  ...
  this.load.image('ground', 'assets/sprites/plataform.png');
...
function create () {
  ...
  piso.create(955, 250, 'ground');
  piso.create(400, 370, 'ground');
  piso.create(250, 650, 'ground');
  piso.create(1220, 100, 'ground');
  ...
  estrelas = this.physics.add.group({
    key: 'star',
    repeat: 20,
    setXY: { x: 250, y: 0, stepX: 75 }
  });

  bombas = this.physics.add.group({
    key: 'bomb',
    repeat: 20,
    setXY: { x: 450, y: 0, stepX: 75}
  });
  ...
  this.physics.add.collider(bombas, piso);

  this.physics.add.overlap(personagem, estrelas, bombas, pegarEstrela, estourarBombas, null, this);
...
function estourarBombas (personagem, bomb) {
